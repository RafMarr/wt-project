...
class DatabaseHelper {
    ...
    /**
     * Returns whether exists a pony with the provided id.
     */
    public function is_pony_id_valid(int $pony_id): bool {
        $query = 'SELECT EXISTS(SELECT 1 FROM ponies WHERE PonyID = ? LIMIT 1)';
        $stmt = $this->db->prepare($query);
        $stmt->bind_param('i', $pony_id);
        $stmt->execute();
    ...
    /**
     * Retrieves the information about the pony with the provided id
     */
    public function get_pony_info(int $pony_id): array {
        $query = 'SELECT PonyID, Name, Breed, HourlyFee, Image, SpecMarks, Description FROM ponies WHERE PonyID = ?';
        $stmt = $this->db->prepare($query);
        $stmt->bind_param('i', $pony_id);
        $stmt->execute();
